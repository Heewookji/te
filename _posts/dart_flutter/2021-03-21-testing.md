...
### Integration Test
앱의 큰 부분 혹은 앱 전체를 테스트한다. 이 테스트의 목표는 모든 위젯과 서비스들이 함께 잘 동작하는지 검증하는 것이다.
...
이 한 쌍의 작업을 하기 위해서, [flutter_driver](https://api.flutter.dev/flutter/flutter_driver/flutter_driver-library.html) 패키지를 써야한다. 이 것은 `instrumented app`을 만들고 `drive`한다.
